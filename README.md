...
We have a model currently made, but due to the small dataset it is not too good yet. Front end has been implemented using streamlit. To run our program, try "streamlit run app.py". 

If you want to train your own model, download everything from requirements.txt in a virtual environment, delete output\toxsxan_model.pt, then try "python src\CONDA.py". After the training, try "streamlit run app.py". 
...
+ our own created datasets from various games (TBD)
